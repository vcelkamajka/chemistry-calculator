...
# only if a prepared solution has been created. Not designed yet in a c1v1 = c2v2 way (TODO).
...
intro_start = input("Do you have a solution made up? (y/n): ")
if intro_start == "y":
...
if intro_start == "n":
    continue_0 = input("Would you like to continue to solution calculations? (y/n). To access dilution calculations, type (d): ")
    if continue_0 == 'y':
...
def choice():
    answer = input("Is a % solution required (y/n), select (n) for molarity: ")
    if answer == 'y':
        return "% solution"
